...
// here any RText properties can be used
}
maxWidth: 200
text: "Some long long long text"
scrollMode: RScrollText.ScrollMode.Always
}
*/
...
Item {
  ...
  default property Component delegate: RText {
    pointSize: Style.fontSizeS
  }
  ...
  property int scrollMode: RScrollText.ScrollMode.Never
  ...
  property int state: RScrollText.ScrollState.None
  ...
  function resetState() {
    root.state = RScrollText.ScrollState.None;
    scrollContainer.x = 0;
    scrollTimer.restart();
    root.updateState();
  }
  ...
  Timer {
    ...
    onTriggered: {
      root.state = RScrollText.ScrollState.Scrolling;
      root.updateState();
    }
  }
  ...
  function ensureReset() {
    if (state === RScrollText.ScrollState.Scrolling)
      state = RScrollText.ScrollState.Resetting;
  }

  function updateState() {
    if (contentWidth <= root.maxWidth || scrollMode === RScrollText.ScrollMode.Never) {
      state = RScrollText.ScrollState.None;
      return;
    }
    if (scrollMode === RScrollText.ScrollMode.Always) {
      if (hoverArea.containsMouse) {
        ensureReset();
      } else {
        scrollTimer.restart();
      }
    } else if (scrollMode === RScrollText.ScrollMode.Hover) {
      if (hoverArea.containsMouse || forcedHover)
        state = RScrollText.ScrollState.Scrolling;
      else
        ensureReset();
    }
  }
  ...
  RowLayout {
    ...
    Loader {
      ...
      sourceComponent: root.delegate
      Layout.fillHeight: true
      visible: root.state !== RScrollText.ScrollState.None
      onLoaded: {
        this.item.text = root.text;
    ...
    NumberAnimation on x {
      running: root.state === RScrollText.ScrollState.Resetting
      to: 0
    ...
    NumberAnimation on x {
      running: root.state === RScrollText.ScrollState.Scrolling
