...
Popup {
  ...
  function fetchAvailableSchemesFromAPI() {
    ...
    xhr.open("GET", "https://raw.githubusercontent.com/me-osano/nocturnal-colorschemes/main/registry.json");
    xhr.send();
  }
  ...
  function downloadScheme(scheme) {
    ...
    if (cachedBranchSha) {
      ...
      xhr.open("GET", "https://api.github.com/repos/me-osano/nocturnal-colorschemes");
      xhr.send();
    }
  }
  ...
  function getSchemeTree(scheme, branch) {
    ...
    xhr.open("GET", "https://api.github.com/repos/me-osano/nocturnal-colorschemes/git/refs/heads/" + branch);
    xhr.send();
  }
  ...
  function getSchemeTreeWithSha(scheme, branch, sha) {
            ...
            if (response.tree && Array.isArray(response.tree)) {
              ...
              for (var i = 0; i < response.tree.length; i++) {
                ...
                if (item.type === "blob" && item.path.startsWith(scheme.path + "/")) {
                  files.push({
                               "path": item.path,
                               "url": "https://raw.githubusercontent.com/me-osano/nocturnal-colorschemes/" + branch + "/" + item.path,
    ...
    xhr.open("GET", "https://api.github.com/repos/me-osano/nocturnal-colorschemes/git/trees/" + sha + "?recursive=1");
    xhr.send();
  }
  ...
  function getSchemeFilesDirect(scheme) {
    ...
    xhr.open("GET", "https://api.github.com/repos/me-osano/nocturnal-colorschemes/contents/" + scheme.path);
    xhr.send();
  }
  ...
  function deleteScheme(schemeName) {
    ...
    deleteProcess.exited.connect(function (exitCode) {
      if (exitCode === 0) {
        ...
        if (needsReset) {
          Logger.i("ColorSchemeDownload", "Deleted scheme was active, resetting to Nocturnal (default)");
          // Clear the setting immediately so ColorSchemeService won't try to apply the deleted scheme
          Settings.data.colorSchemes.predefinedScheme = "Nocturnal (default)";
          // Apply the default scheme immediately
          ColorSchemeService.setPredefinedScheme("Nocturnal (default)");
        }
