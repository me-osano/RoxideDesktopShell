...
ColumnLayout {
  ...
  Component.onCompleted: {
    ...
    if (root.isGitVersion) {
      ...
      if (HostService.isNixOS) {
        ...
        if (shellDir) {
          // Extract commit hash from path like: /nix/store/...-nocturnal-shell-2025-11-30_225e6d3/share/nocturnal-shell
          // Pattern matches: nocturnal-shell-YYYY-MM-DD_<commit_hash>
          var match = shellDir.match(/nocturnal-shell-\d{4}-\d{2}-\d{2}_([0-9a-f]{7,})/i);
          if (match && match[1]) {
  ...
  Process {
    id: pacmanProcess
    command: ["pacman", "-Q", "nocturnal-shell-git"]
    running: false
    ...
    onExited: function (exitCode) {
      ...
      if (exitCode === 0) {
        ...
        var match = output.match(/nocturnal-shell-git\s+(.+)/);
        if (match && match[1]) {
      ...
      GridLayout {
        ...
        RText {
          ...
          MouseArea {
            ...
            onClicked: {
              if (root.commitInfo) {
                Quickshell.execDetached(["xdg-open", "https://github.com/me-osano/nocturnal-shell/commit/" + root.commitInfo]);
              }
            }
          }
        }
