...
Scope {
  ...
  property string pamConfig: Quickshell.env("NOCTURNAL_PAM_SERVICE") || "login"
  property bool pamReady: false

  Component.onCompleted: {
    if (Quickshell.env("NOCTURNAL_PAM_SERVICE")) {
      Logger.i("LockContext", "NOCTURNAL_PAM_SERVICE is set, using system PAM config: /etc/pam.d/" + pamConfig);
      pamReady = true;
    } else {
