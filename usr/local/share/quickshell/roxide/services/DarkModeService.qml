...
Singleton {
  ...
  Connections {
    ...
    function onWeatherReadyChanged() {
      if (WeatherService.weatherReady) {
        const changes = root.collectWeatherChanges(WeatherService.weather);
  ...
  function update() {
    ...
    } else if (Settings.data.colorSchemes.schedulingMode == "location" && WeatherService.weather) {
      const changes = collectWeatherChanges(WeatherService.weather);
      initComplete = true;
      applyCurrentMode(changes);
      scheduleNextMode(changes);
    }
