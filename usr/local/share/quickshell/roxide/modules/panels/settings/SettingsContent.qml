...
Item {
  ...
  // Set sub-tab on the currently loaded tab content. Returns true if an RTabBar was found.
  ...
  function setSubTabRecursive(item, subTabIndex) {
    ...
    if (item.objectName === "RTabBar") {
      // Prepare the sibling RTabView so the index change doesn't animate
      if (item.parent) {
        for (let j = 0; j < item.parent.children.length; j++) {
          const sibling = item.parent.children[j];
          if (sibling.objectName === "RTabView" && sibling.setIndexWithoutAnimation) {
